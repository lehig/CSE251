...
def solve_with_threads(maze, prev, curr, visited:set, color, threads):
    global stop
    global thread_count
    if not stop and curr not in visited:
        visited.add(curr[0], curr[1], color)
        if (maze.at_end(curr[0], curr[1])):
