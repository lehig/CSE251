...
def main():
    ...
    # getting the top url
    response = pool.apply_async(request_func, args=(TOP_API_URL))
    ...
    characters = [pool.apply_async(request_func, args=url, callback=get_name) for url in response.get()['characters']]
    planets = [pool.apply_async(request_func, args=url, callback=get_name) for url in response.get()['planets']]
    starships = [pool.apply_async(request_func, args=url, callback=get_name) for url in response.get()['starships']]
    vehicles = [pool.apply_async(request_func, args=url, callback=get_name) for url in response.get()['vehicles']]
    species = [pool.apply_async(request_func, args=url, callback=get_name) for url in response.get()['species']]
